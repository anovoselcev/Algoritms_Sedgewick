...
template<typename T>
size_t LengthOfExternalPath(Node<T>* root, size_t level = 0) {
    if (!root) return 0;
    if (!root->left && !root->right) return level;
    return LengthOfExternalPath(root->left, level + 1) + LengthOfExternalPath(root->right, level + 1);
}

int main() {
    std::cout << LengthOfExternalPath(CreateTree<int>(100)) << std::endl;
}